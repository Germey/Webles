...
<tr><td width=30><img src=images/admin_top_close.gif></td><td style="font-weight:800;color:white"> 编辑文章</td></tr>
</table>
...
<option value='del'>删除</option><option value='delall'>全部删除</option><option value='check'>通过审核</option><option value='uncheck'>取消通过</option><option value='checkall'>全部通过</option><option value='settop'>固顶</option><option value='unsettop'>解固</option><option value='sethot'>设为热点</option><option value='unsethot'>解除热点</option><option value='setrec'>设为推荐</option><option value='unsetrec'>解除推荐</option></select> <input type=button value='执行' onclick="checkAction(document.myform.selection.options[document.myform.selection.selectedIndex].value)">
