...
<tr><td width=30><img src=images/admin_top_close.gif></td><td style="font-weight:800;color:white"> 编辑文章<font color=#cc0000>（点击文章标题可以修改）</font></td></tr>
</table>
...
if($classid==null)
{
$res=mysql_query("select count(*) from article as a inner join articleclass as c on a.classid=c.classid where a.classid IN ($admin_classid) and a.deleted=0 and a.title like '%$keyword%' and a.shenhe like '%$shenhe%' ");
}
else
{
$res=mysql_query("select count(*) from article as a inner join articleclass as c on a.classid=c.classid where a.classid IN ($admin_classid) and a.deleted=0 and a.title like '%$keyword%' and a.shenhe like '%$shenhe%' and (a.classid='$classid' or c.parentid='$classid') ");
}
...
if($classid==null)
{
$res=mysql_query("select * from article as a inner join articleclass as c on a.classid=c.classid where a.classid IN ($admin_classid) and a.deleted=0 and a.title like '%$keyword%' and a.shenhe like '%$shenhe%'  order by a.istop desc,a.paixu desc,a.adddate desc,a.articleid desc limit $offset,$pagesize");
}
else
{
$res=mysql_query("select * from article as a inner join articleclass as c on a.classid=c.classid where a.classid IN ($admin_classid) and a.deleted=0 and a.title like '%$keyword%' and a.shenhe like '%$shenhe%' and (a.classid='$classid' or c.parentid='$classid') order by a.istop desc,a.paixu desc,a.adddate desc,a.articleid desc limit $offset,$pagesize");
}
...
<td background=images/topbar_bg.gif align=center width='10%'>排序</td>
...
<td>
<a href="edit_art.php?articleid=<?echo $r["articleid"];?>">
<?
if($r["ispic"]==1)
{
?><font color=red>[图]</font><?}?><?echo $r["title"];?></a></td>
...
<td align=center><?echo $r["paixu"];?></td>
<td align=center>
...
<?if($page>1){echo "<a href='?page=".$first."&keyword=".$keyword."&shenhe=".$shenhe."&classid=".$classid."&type=".$type."&ctype=".$ctype."'>"?><font face="Webdings">9</font></a>&nbsp;<?echo "<a href='?page=".$prev."&keyword=".$keyword."&shenhe=".$shenhe."&classid=".$classid."&type=".$type."&ctype=".$ctype."'>"?><font face="Webdings">7</font></a><?}else{?><font face="Webdings">9</font>&nbsp;<font face="Webdings">7</font><?}?>
...
{ 
$pagetitle=$pagetitle." "."["."<a href='?page=".$j."&keyword=".$keyword."&shenhe=".$shenhe."&classid=".$classid."&type=".$type."&ctype=".$ctype."'>".$j."</a>"."]" ;
}
}
...
<option value='del'>放入回收站</option><option value='check'>通过审核</option><option value='uncheck'>取消通过</option><option value='settop'>固顶</option><option value='unsettop'>解固</option><option value='sethot'>设为热点</option><option value='unsethot'>解除热点</option><option value='setrec'>设为推荐</option><option value='unsetrec'>解除推荐</option></select> <input type=button value='执行' onclick="checkAction(document.myform.selection.options[document.myform.selection.selectedIndex].value)">
